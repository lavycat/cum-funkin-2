...
func get_song_meta(i:int) -> SongMeta:
	...
	if ResourceLoader.exists(meta_path):
		...
		return SongMeta.new()
var cap:PackedScene = load("res://scenes/menus/vslice_freeplay/WIP_VSLICE_FREEPLAY_CAP.tscn")
func update_caps():
	for s in songs.get_child_count():
		var capp:FreeplayItem = songs.get_child(s)
		capp.target_position = capp.get_pos(s - cur_selected)
		capp.deselect()
	songs.get_child(cur_selected).select()
...
func _ready() -> void:
	for i in list.size():
		...
		var capp:FreeplayItem = cap.instantiate()
		capp.data = FreeplayItemData.new()
		capp.data.name = s
		capp.target_position = capp.get_pos(i)
		if meta.icon:
			capp.data.icon = meta.icon
		songs.add_child(capp)
		#var t := Label.new()
		#t.text = s.to_upper()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("ui_accept"):
		var capp:FreeplayItem = songs.get_child(cur_selected)
		capp.confirm()
		dj.confirm()
		AudioManager.play_sfx(1)
		await get_tree().create_timer(1,5,false).timeout
		AudioManager.fade_out_global_music(0.5)
